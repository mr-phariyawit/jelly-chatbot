...
exports.processITSupportMessage = processITSupportMessage;
exports.processMessage = processITSupportMessage;
 ...
 */
async function processITSupportMessage(input) {
    try {
        ...
        const conversationContext = messages.map(m => `${m.role === 'user' ? 'ผู้ใช้' : 'AI'}: ${m.content}`).join('\n');
...
กรุณาตอบในฐานะ AI Support Assistant:`;
        ...
        const userLogResult = await (0, session_1.logUserMessage)(input.userId, input.content);
        ...
        if (shouldEscalate) {
            ...
            const assistantLogResult = await (0, session_1.logAssistantMessage)(input.userId, finalMessage, true, input.content.substring(0, 100));
            ...
            await (0, session_1.logAssistantMessage)(input.userId, finalMessage, false);
        }
